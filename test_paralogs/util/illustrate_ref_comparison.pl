...
my $usage = "usage: $0 refseqs.fa target.fa min_per_id\n\n";
...
main: {
    ...
    my $cmd = "blat $TARGET_FA $REFSEQS_FA -t=dna -q=dna -out=blast9 blat.blastn.out";
    ...
    &generate_ascii_illustration("blat.blastn.out");
...
sub generate_ascii_illustration {
    ...
    foreach my $ref_acc (keys %ref_seq_lengths) {
        ...
        foreach my $match (@matches) {
            ...
            my $pct_of_target_aligned = sprintf("%.2f", ($target_end-$target_start + 1) / $target_length * 100);
