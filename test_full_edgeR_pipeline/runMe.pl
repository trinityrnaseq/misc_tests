...
## Run Trinity:
my $cmd = "$BASEDIR/../../trunk/Trinity.pl --seqType fq --SS_lib_type RF --JM 1G "
...
foreach my $condition (@conditions) {
    ...
    my $cmd = "$BASEDIR/../../trunk/util/alignReads.pl --target trinity_out_dir/Trinity.fasta "
...
foreach my $condition (@conditions) {
    ...
    ## run RSEM
    my $cmd = "$BASEDIR/../../trunk/util/RSEM_util/run_RSEM.pl --transcripts target.fa "
...
# make iso matrix
$cmd = "$BASEDIR/../../trunk/util/RSEM_util/merge_RSEM_frag_counts_single_table.pl " . join (" " , @iso_links) . " > Trinity_trans.counts.matrix";
&process_cmd($cmd) unless (-s "Trinity_trans.counts.matrix");

# make the 'gene' matrix
$cmd = "$BASEDIR/../../trunk/util/RSEM_util/merge_RSEM_frag_counts_single_table.pl " . join (" " , @gene_links) . " > Trinity_components.counts.matrix"; 
...
foreach my $target_type ("trans", "components") {
    ...
    my $cmd = "$BASEDIR/../../trunk/Analysis/DifferentialExpression/run_EdgeR.pl --matrix Trinity_${target_type}.counts.matrix "
