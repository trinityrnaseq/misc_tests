...
# run Trinity.
&process_cmd("Trinity.pl --seqType fq --SS_lib_type RF --left ALL.LEFT.fq --right ALL.RIGHT.fq --CPU 4 --JM 1G") unless (-s "trinity_out_dir/Trinity.fasta");
...
foreach my $sample (@samples) {
    ...
    unless (-s $rsem_result_file) {
        ...
        &process_cmd("$trinity_dir/util/RSEM_util/run_RSEM_align_n_estimate.pl --seqType fq --left $sample.left.fq --right $sample.right.fq --transcripts trinity_out_dir/Trinity.fasta --prefix $sample -- --no-bam-output");
...
&process_cmd("$trinity_dir/util/RSEM_util/merge_RSEM_frag_counts_single_table.pl  @rsem_result_files > transcripts.counts.matrix");
...
&process_cmd("head edgeR/transcripts.counts.matrix.Sp_log_vs_Sp_plat.edgeR.DE_results");

&show("edgeR/transcripts.counts.matrix.Sp_log_vs_Sp_plat.edgeR.DE_results.MA_n_Volcano.pdf");

&process_cmd("sed '1,1d' edgeR/transcripts.counts.matrix.Sp_log_vs_Sp_plat.edgeR.DE_results | awk '{ if ($5 <= 0.05) print;}' | wc -l");
...
&process_cmd("wc -l diffExpr.P0.001_C2.matrix"); # number of DE transcripts + 1
...
&process_cmd("$trinity_dir/Analysis/DifferentialExpression/define_clusters_by_cutting_tree.pl --Ptree 60 -R diffExpr.P0.001_C2.matrix.R.all.RData");
&show("diffExpr.P0.001_C2.matrix.R.all.RData.clusters_fixed_P_60/my_cluster_plots.pdf");
