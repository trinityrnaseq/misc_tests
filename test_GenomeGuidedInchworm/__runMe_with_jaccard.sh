...
../../trunk/util/prep_rnaseq_alignments_for_genome_assisted_assembly.pl --coord_sorted_SAM bowtie_out.coordSorted.sam -J 1 -I 500 --SS_lib_type RF --jaccard
...
../../trunk/util/write_inchworm_cmds.pl input.Read_files > iworm.cmds

## Run inchworm assemblies in parallel

../../trunk/Inchworm/bin/ParaFly -c iworm.cmds -CPU 4 -failed_cmds iworm.cmds.failed -v

## Combine all inchworm assemblies into a single output file, and assign unique accessions to each assembled contig.

find Dir* -type f -name "*iworm" -exec cat {} + | ../../trunk/util/inchworm_accession_incrementer.pl > iworm.GenomeGuided.fasta
